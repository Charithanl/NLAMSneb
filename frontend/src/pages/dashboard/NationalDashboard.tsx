...
export function NationalDashboard() {
  ...
  useEffect(() => {
    void aiService.loadSummary().then(setAiSummary).catch(() => setAiSummary(null));
  }, []);
  const quickStats = useMemo(
    () => [
      ...
      {
        label: "Open risks",
        value: aiSummary ? String(aiSummary.criticalRisks) : "…",
        detail: "Needs attention",
        icon: Radar,
        tone: "rose" as const,
      },
    ],
    [aiSummary, compensationSummary.totalCases, parcelStats.totalParcels, projectStats.totalProjects, rrSummary.totalFamilies],
  );
    ...
    <div className="space-y-7">
      <InfoRibbon
        ...
        items={[
          ...
          { label: "Risks", value: aiSummary ? String(aiSummary.criticalRisks) : "…" },
        ]}
      />
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <AppCard title="Today at a glance" description="Very short operational signals.">
          <div className="grid gap-3 sm:grid-cols-2">
            <MiniSignal label="Critical risks" value={aiSummary ? `${aiSummary.criticalRisks}` : "…"} />
